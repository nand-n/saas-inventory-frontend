...
import { CheckCircle, XCircle, Eye, Clock, AlertCircle, FileText, Calendar, Globe, DollarSign } from "lucide-react";
...
interface CustomsDocumentDetailProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function CustomsDocumentDetail({
  isOpen,
  onClose,
  document,
  onStatusChange,
}: CustomsDocumentDetailProps) {
  ...
  const handleReview = async () => {
    ...
    try {
      await reviewCustomsDocument(document.id!, "current-user-id", reviewNotes);
      toast({
  ...
  const handleApprove = async () => {
    setIsSubmitting(true);
    try {
      await approveCustomsDocument(document.id!, "current-user-id", reviewNotes);
      toast({
  ...
  const handleReject = async () => {
    ...
    setIsSubmitting(true);
    try {
      await rejectCustomsDocument(document.id!, "current-user-id", rejectionReason);
      toast({
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">{document.documentNumber}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-base">{document.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-muted-foreground">Requires Approval</Label>
                  <p className="text-base">{document.requiresApproval ? "Yes" : "No"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Dates & Authority</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium text-muted-foreground">Issued Date</Label>
                  <p className="text-base">{formatDate(document.issuedDate)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-base">{document.issuingAuthority}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-muted-foreground">Issuing Country</Label>
                  <p className="text-base">{document.issuingCountry}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {(document.declaredValue || document.currency) && (
            <Card>
              <CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {document.declaredValue && (
                    <div>
                      ...
                      <p className="text-base">
                        {document.currency} {Number(document.declaredValue).toLocaleString()}
                      </p>
                    </div>
                  )}
                  {document.currency && (
              ...
              <CardContent className="space-y-4">
                {document.description && (
                  <div>
                    ...
                    <p className="text-base mt-1">{document.description}</p>
                  </div>
                )}
                {document.notes && (
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Notes</Label>
                    <p className="text-base mt-1">{document.notes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
